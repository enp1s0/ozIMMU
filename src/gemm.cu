...
void matmul_core(
		...
		const std::size_t ld_w_a,
		const void* const b_working_memory_ptr,
		const std::size_t ld_w_b
		) {
	...
	for (unsigned i = 0; i < gemm_pair_config.A_id; i++) {
		...
		A_working_ptr_offset += num_int8_a_slice_elements * mtk::ozimmu::get_data_size_in_byte(t);
	}
	...
	for (unsigned i = 0; i < gemm_pair_config.B_id; i++) {
		...
		B_working_ptr_offset += num_int8_b_slice_elements * mtk::ozimmu::get_data_size_in_byte(t);
	}
...
int gemm_int8<double>(
	...
	for (const auto& gemm_pair_config : gemm_pair_config_list) {
		matmul_core(
				handle,
				op_A, op_B,
				m, n, ld_int8_a, // use ld_int8_a instead of k for better stability
...
int gemm_int8<cuDoubleComplex>(
	...
	std::int32_t* const c_i32_ptr = reinterpret_cast<std::int32_t*>(b_imag_max_exp_ptr + n);
	void* const working_memory_ptr = c_i32_ptr + m * n;
...
int mtk::ozimmu::gemm(
	...
	if (element_kind == mtk::ozimmu::real) {
		arg_error |= check_address_alignment<double>(reinterpret_cast<const double*>(a_ptr), "A");
		arg_error |= check_address_alignment<double>(reinterpret_cast<const double*>(b_ptr), "B");
		arg_error |= check_address_alignment<double>(reinterpret_cast<const double*>(c_ptr), "B");
	} else {
		arg_error |= check_address_alignment<cuDoubleComplex>(reinterpret_cast<const cuDoubleComplex*>(a_ptr), "A");
		arg_error |= check_address_alignment<cuDoubleComplex>(reinterpret_cast<const cuDoubleComplex*>(b_ptr), "B");
		arg_error |= check_address_alignment<cuDoubleComplex>(reinterpret_cast<const cuDoubleComplex*>(c_ptr), "B");
	}
	if (arg_error) {
		return 1;
	}
	...
	gemm_list_t gemm_list = {
		std::tuple<mtk::ozimmu::operation_t, mtk::ozimmu::operation_t, std::size_t, std::size_t, std::size_t, mtk::ozimmu::element_kind_t, mtk::ozimmu::compute_mode_t>{op_A, op_B, m, n, k, element_kind, compute_mode}
	};
